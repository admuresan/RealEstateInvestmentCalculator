...
function initFeatureRequestButton() {
    ...
    if (footer) {
        function updateButtonPosition() {
            ...
            const shouldBeFixed = windowHeight > documentHeight;
            const isFixed = footer.classList.contains('fixed-bottom');
            if (shouldBeFixed !== isFixed) {
                footer.classList.toggle('fixed-bottom', shouldBeFixed);
            }
        }
        ...
        // Throttled update when content changes. Disconnect before modifying to avoid
        // feedback loop (our class change would re-trigger the observer and freeze the page)
        let rafId = null;
        let observer = null;
